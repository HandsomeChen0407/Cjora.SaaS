...
internal sealed class DepartmentAppService : IDepartmentAppService
{
    ...
    /// </summary>
    private async Task RebuildClosureForNodeAsync(long nodeId, long? parentId, CancellationToken cancellationToken)
    {
    ...
    {
        var all = await _departments.GetListAsync(cancellationToken);
        await _db.Deleteable<SysDepartmentClosure>().ExecuteCommandAsync(cancellationToken);
