...
public sealed class SysTenantRepository : ISysTenantRepository
{
    ...
    public async Task InsertAsync(SysTenant tenant, CancellationToken cancellationToken = default)
    {
        ...
        await _databaseClient.Insertable(tenant).ExecuteCommandAsync(cancellationToken);
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(tenant);
        await _databaseClient.Updateable(tenant).ExecuteCommandAsync(cancellationToken);
    }
}
